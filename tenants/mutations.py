...
async def _check_client_or_spark_admin(request_user):
    ...
    # Shared, email-aware resolver — re-reads role by PK/email and treats any
    # @igniteproductions.co user (and is_staff/superuser) as a platform admin.
    from utils.graphql.permissions import (
        resolve_request_user_access,
        _is_admin_access,
    )

    role_slug, _is_staff, _is_super, _email = await resolve_request_user_access(
        request_user
    )
    is_platform_admin = _is_admin_access(role_slug, _is_staff, _is_super, _email)
