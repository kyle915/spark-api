"""HTTP entry for Torch retail-schedule Sheet → confirmation / cancel.

POST ``/internal/torch-sheet-event-confirmation`` with ``X-Cron-Secret``.
...
def _parse_body(request: HttpRequest) -> dict:
    if request.body:
        try:
            ...
            pass
    # Form-encoded fallback for quick curl tests.
    return {k: request.POST.get(k) for k in request.POST.keys()}


@method_decorator(csrf_exempt, name="dispatch")
class TorchSheetEventConfirmationView(View):
    """POST body::

        {
          "action": "send" | "cancel",
          "rowNumber": 42,
          "sheetId": "<optional; must be Torch retail schedule>",
          "dryRun": false,
          "values": { "Date": "...", "BA Name": "...", ... }
        }
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        sheet_id = (
            body.get("sheetId") or body.get("sheet_id") or TORCH_PUBLIC_FORM_SHEET_ID
        )
        try:
        ...
        try:
            ...
            logger.exception("torch-sheet-event-confirmation failed")
            return JsonResponse(
