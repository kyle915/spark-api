...
class CustomRecapTemplateQueriesService(SparkGraphQLMixin):
    ...
    async def get_record(
        ...
        """Return a single CustomRecapTemplate by id or uuid."""
        filters: dict[str, object] = {}
        if id not in (None, ""):
            filters["id"] = id
        if uuid not in (None, ""):
...
class RecapQueries:
    ...
    async def custom_recaps(
        ...
        resolved_event_id = (
            resolve_id_to_int(event_id) if event_id not in (None, "") else None
        )
        resolved_custom_recap_template_id = (
            resolve_id_to_int(custom_recap_template_id)
            if custom_recap_template_id not in (None, "")
            else None
        )
        resolved_ambassador_id = (
            resolve_id_to_int(ambassador_id)
