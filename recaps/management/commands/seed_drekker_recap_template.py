"""Seed Drekker Brewing Retail Sampling recap template.

Mirrors Liquid Death Retail Sampling field-for-field (see live LD PDF /
Brew Dr retail seed), with Drekker brand copy and Products Sampled options
from the live Drekker Product catalog (no hardcoded second SKU list).
GraphQL also refreshes Products Sampled from the catalog at read time.

Photos stay on the walk-up ``FileRecapCategory`` buckets from
``setup_drekker_checkin``; this SPEC does NOT add template image fields.

DRY-RUN by default. Run via ``/internal/cron/seed-drekker-recap-template``
(or the "Seed Drekker recap template" GitHub Action) against prod.
"""
...
RETAIL_TEMPLATE_NAME = "Drekker Brewing-Retail Sampling"
...
# Field-for-field off Liquid Death-Retail Sampling, Drekker brand copy.
# Products Sampled options are filled at seed time from the tenant catalog.
RETAIL_SPEC_FIELDS: list[tuple[str, list[tuple[str, str, bool]]]] = [
    (
        "Consumer Engagement",
        [
...
def build_retail_spec(product_opts: list[str]) -> list[
    tuple[str, list[tuple[str, str, bool, list[str]]]]
]:
    """Assemble RETAIL_SPEC with catalog-backed Products Sampled options."""
    sections: list[tuple[str, list[tuple[str, str, bool, list[str]]]]] = []
    for section_name, fields in RETAIL_SPEC_FIELDS:
        sections.append(
            (
                section_name,
...
class Command(BaseCommand):
    help = (
        "Seed Drekker Brewing's LD-mirrored Retail Sampling recap template "
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--tenant",
            default="drekker",
            help="tenant name/slug substring (case-insensitive). Default: 'drekker'.",
        )
        parser.add_argument(
            "--template-name",
            dest="template_name",
    ...
    def _resolve_tenant(self, needle: str):
        ...
        if not matches:
            raise CommandError(
                f"No tenant matches {needle!r}. Drekker needs onboarding first."
            )
        raise CommandError(
    ...
    def _product_options(self, tenant) -> list[str]:
        ...
        if opts:
            self.stdout.write(
                f"Products   : {len(opts)} from Drekker Product catalog"
            )
        else:
            self.stdout.write(
                self.style.WARNING(
    ...
    def _seed_program(
        ...
        spec = build_retail_spec(product_opts)
    ...
    def handle(self, *args, **opts):
        ...
        self.stdout.write(
            "Programs   : " + ", ".join(p["event_type"] for p in programs)
        )
        ...
        self.stdout.write("=" * 68)
        self.stdout.write(
            "Mirrors Liquid Death Retail Sampling "
            "(Drekker brand copy; Products Sampled = catalog)."
        )
        ...
        def _run():
            for program in programs:
                self._seed_program(
                    tenant, creator, apply, program, ft_cache, product_opts
                )
