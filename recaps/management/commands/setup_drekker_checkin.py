"""Make Drekker Brewing's ONE standing check-in link serve Retail Sampling.

Mirrors ``setup_ld_retail_checkin`` / ``setup_brew_dr_checkin`` for a
retail-only brand: Liquid Death retail photo buckets on a durable ``DR-``
URL. Recap form is seeded by ``seed_drekker_recap_template`` — this command
creates NEITHER the template nor a new code when one already exists.

DRY-RUN by default. Run via ``/internal/cron/setup-drekker-checkin`` (or the
"Setup Drekker check-in" GitHub Action) so it executes against prod.
"""
...
# Liquid Death retail shot list (Table / Display / Consumer / Receipt).
...
class Command(BaseCommand):
    help = (
        "Make Drekker Brewing's standing check-in link serve Retail Sampling "
        "(photo buckets + selectable type; dry-run default)."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default="",
            help=(
                "brand prefix for a NEWLY minted code, e.g. 'DR' -> "
                f"DR-XXXXXX. Blank keeps {CODE_PREFIX!r}."
            ),
        )
        parser.add_argument(
            "--code",
    ...
    def _mint_code(self, prefix: str) -> str:
        ...
        if raw and not 1 <= len(cleaned) <= 4:
            raise CommandError("--prefix should be 1-4 letters/digits, e.g. DR.")
