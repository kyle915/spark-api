"""Set up Brand Revolution: tenant + Event Activation recap + standing check-in.

Generic Event Activation walk-up (UT-Austin activations): two count fields
plus a Recap pictures multi-upload bucket (library + camera). Recaps stay
admin-approved / Needs review — walk-up submits leave approved=False.

DRY-RUN by default. Run via ``/internal/cron/setup-brand-revolution-checkin``
(or the "Setup Brand Revolution check-in" GitHub Action) against prod.
"""
...
TENANT_NAME = "Brand Revolution"
TENANT_SLUG = "brand-revolution"
TEMPLATE_NAME = "Brand Revolution-Event Activation"
CODE_PREFIX = "BR-"
PROGRAM_NAME = "Event Activation"

# Exactly the metrics Kyle asked for. Date / venue come from clock-in find-or-
# create (BA types UT-Austin / GPS). Photos are a labelled bucket — not a
# template image field — so walk-up keeps library + camera multi-upload.
SPEC: list[tuple[str, list[tuple[str, str, bool, list[str]]]]] = [
    (
        "Consumer Engagement",
        [
            ("How many people did you interact with?", "number", True, []),
            ("How many people did you survey?", "number", True, []),
        ],
    ),
]

PHOTO_BUCKETS: list[dict] = [
    {"name": "Recap pictures", "min": 1},
]


class Command(BaseCommand):
    help = (
        "Set up Brand Revolution: tenant (if missing), Event Activation "
        "recap template, Recap pictures bucket, and standing check-in link "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default=None,
            help=(
                "event type name substring. Default: Event Activation "
                "(Brand Revolution is a single program)."
            ),
        )
        parser.add_argument(
            "--prefix",
            ...
            default="",
            help=(
                "brand prefix for a NEWLY minted code, e.g. 'BR' -> "
                f"BR-XXXXXX. Blank keeps {CODE_PREFIX!r}."
            ),
        )
        parser.add_argument(
            "--apply",
    ...
    def _print_spec(self) -> None:
        ...
        for bucket in PHOTO_BUCKETS:
            hint = f" (min {bucket['min']})" if bucket.get("min") else ""
            self.stdout.write(f"    - {bucket['name']!r}{hint}")
    ...
    def _resolve_or_create_tenant(self, needle: str, creator, apply: bool):
        ...
        matches = list(
            Tenant.objects.filter(
                ...
                | Q(name__icontains="brand revolution")
            )
            .distinct()
            .order_by("id")
        )
    ...
    def _resolve_event_type(self, tenant, hint: str | None, creator, apply: bool):
        ...
        # Activation. Create the program when missing (DAOU-style).
        if not apply:
    ...
    def _photo_buckets(self, tenant, creator, apply: bool) -> None:
        ...
        # Key by program so list-checkin-links / multi-program tenants stay clear.
        wanted = {PROGRAM_NAME: PHOTO_BUCKETS}
    ...
    def _retire_other_programs(self, tenant, event_type, apply: bool) -> None:
        ...
        if not others:
            self.stdout.write(
                "\nNo other event types — Event Activation is the only program."
            )
            return
    ...
    def _checkin_code(self, tenant, apply: bool, prefix: str = "") -> None:
        ...
        if raw and not 1 <= len(cleaned) <= 4:
            raise CommandError("--prefix should be 1-4 letters/digits, e.g. BR.")
