"""Set up Luckin Coffee: tenant + recap template + standing check-in.

Seeds the **Luckin Coffee // Sampling Recap** form field-for-field off the
client's Connecteam PDF exports (Aurora Diaz / Niamani Charra, Jul–Aug 2026).
Date and Store / Account Location are NOT template fields — clock-in on the
standing link captures those on the event.

DRY-RUN by default. Run via ``/internal/cron/setup-luckin-checkin`` (or the
"Setup Luckin Coffee check-in" GitHub Action) so it executes against prod.
"""
...
TENANT_NAME = "Luckin Coffee"
TENANT_SLUG = "luckin-coffee"
TEMPLATE_NAME = "Luckin Coffee · Sampling Recap"
CODE_PREFIX = "LC-"
PROGRAM_NAME = "Event"
...
SPEC: list[tuple[str, list[tuple[str, str, bool, list[str]]]]] = [
    (
        "Activation Metrics",
        [
            ("Walk-by traffic", "text", True, []),
            ("Number of people approached and engaged", "number", True, []),
            ("Number of App download QR scans", "number", True, []),
            ("Number of store visits", "number", True, []),
            ("Number of merch distributed", "number", True, []),
            ("Remaining merch", "text", True, []),
            ("Best-performing time window", "text", True, []),
        ],
    ),
    (
        "Feedback",
        [
            ("Key customer feedback/issues", "longtext", True, []),
            ("Customer profile", "longtext", True, []),
        ],
    ),
]

PHOTO_BUCKETS: list[dict] = [
    {"name": "Sampling Pictures"},
]


class Command(BaseCommand):
    help = (
        "Set up Luckin Coffee: tenant (if missing), Sampling Recap template, "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            help=(
                ...
                "(Luckin Coffee is a single program)."
            ),
        )
        parser.add_argument(
            "--prefix",
            ...
            default="",
            help=(
                "brand prefix for a NEWLY minted code, e.g. 'LC' -> "
                f"LC-XXXXXX. Blank keeps {CODE_PREFIX!r}."
            ),
        )
        parser.add_argument(
            "--apply",
    ...
    def _checkin_code(self, tenant, apply: bool, prefix: str = "") -> None:
        ...
        if raw and not 1 <= len(cleaned) <= 4:
            raise CommandError("--prefix should be 1-4 letters/digits, e.g. LC.")
