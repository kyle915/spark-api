"""Seed DAOU Event Activation recap template for Treasury Wine Estates.

Mirrors Liquid Death Event Activation field-for-field (see LD Event PDF),
with DAOU brand copy and Products Sampled options from the live Treasury /
DAOU Product catalog (no hardcoded second SKU list). GraphQL also refreshes
Products Sampled from the catalog at read time.

Photos stay on the walk-up ``FileRecapCategory`` buckets from
``setup_daou_checkin``; this SPEC does NOT add template image fields.

DRY-RUN by default. Run via ``/internal/cron/seed-daou-recap-template``
(or the "Seed DAOU recap template" GitHub Action) against prod.
"""
...
EVENT_TEMPLATE_NAME = "DAOU-Event Activation"
...
# Field-for-field off Liquid Death Event Activation, DAOU brand copy.
...
EVENT_SPEC_FIELDS: list[tuple[str, list[tuple[str, str, bool]]]] = [
    (
        ...
        [
            ...
            (
                "How many consumers had tried a DAOU wine before?",
                "number",
                True,
            ),
...
class Command(BaseCommand):
    ...
    def _product_options(self, tenant) -> list[str]:
        ...
        if opts:
            self.stdout.write(
                f"Products   : {len(opts)} from DAOU Product catalog"
            )
        else:
            self.stdout.write(
                self.style.WARNING(
    ...
    def handle(self, *args, **opts):
        ...
        if apply:
            self.stdout.write(
                self.style.SUCCESS(
                    f"APPLIED — seeded {len(programs)} DAOU "
