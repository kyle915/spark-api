"""Set up G7 Entertainment Marketing: tenant + recap template + standing check-in.

The G7 twin of ``setup_krispy_krunchy_checkin`` — same two things in one
idempotent command, plus tenant creation because this brand does not exist
yet:

1. The **BA Event Recap** custom template, field-for-field off the client's
   own "BA EVENT RECAP" PDF (Leah Love, #1446, 08/19/2026). Date and Event
   Location are NOT template fields: clock-in on the standing link already
   captures where/when the BA started, and the recap PDF already renders
   date/location from the event. Event Name stays — the activation name
   (e.g. "Wilson County Fair") is not the GPS address.
...
how Liquid Death / KKC keep those dropzones apart, each with library +
camera multi-upload.

Recaps stay human-reviewed (not Feel Free auto-approve). Location mode is
Event-style address find-or-create, same as KKC — this is a BA event
activation recap, not a market/sample-stop or typed-store agency link.

DRY-RUN by default. Run via ``/internal/cron/setup-g7-entertainment-checkin``
(or the "Setup G7 Entertainment Marketing check-in" GitHub Action) so it
...
TENANT_NAME = "G7 Entertainment Marketing"
TENANT_SLUG = "g7-entertainment"
TEMPLATE_NAME = "G7 Entertainment Marketing · BA Event Recap"
CODE_PREFIX = "G7-"
# G7 runs one program. Clock-in, request, and walk-up all stamp this —
...
class Command(BaseCommand):
    help = (
        "Set up G7 Entertainment Marketing: tenant (if missing), BA Event "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default="",
            help=(
                "brand prefix for a NEWLY minted code, e.g. 'G7' -> "
                f"G7-XXXXXX. Blank keeps {CODE_PREFIX!r}."
            ),
        )
        parser.add_argument(
            "--apply",
    ...
    def _resolve_event_type(self, tenant, hint: str | None, creator, apply: bool):
        ...
        # Event, not sampling — G7 is one program. Preferring "sampling"
    ...
    def _checkin_code(self, tenant, apply: bool, prefix: str = "") -> None:
        ...
        # Digits stay (G7); drop punctuation only. isalpha() would mint G-XXXXXX.
        cleaned = "".join(ch for ch in raw if ch.isalnum())
        if raw and not 1 <= len(cleaned) <= 4:
            raise CommandError("--prefix should be 1-4 letters/digits, e.g. G7.")
