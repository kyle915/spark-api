"""Set up Dude Wipes: tenant + Event recap template + standing check-in.

Mirrors the Breakaway White Claw Surge (WCS_SPEC) structure and layout,
re-branded field-for-field for Dude Wipes (Tenant ID: 15, slug: dude-wipes):
- Template: "Dude Wipes · Event Recap"
- Sections: Event Details, Sampling Counts, Feedback
- Metrics: samples handed out / distributed, consumer interaction count,
  foot traffic / impressions, key highlights, consumer feedback / quotes,
  brand awareness percent ("What percent of consumers had heard of or tried
  Dude Wipes before?"), consumer sentiment, and event notes.
- Photo buckets: Consumer Sampling Pictures, Event Setup / Footprint.
- Standing walk-up check-in code: durable DW-XXXXXX link pinned to Event.

Recaps stay human-reviewed (approved=False until admin approve).
Idempotent: an existing checkin_code is never rotated.

DRY-RUN by default. Run via ``/internal/cron/setup-dude-wipes-checkin`` (or
the "Setup Dude Wipes check-in" GitHub Action) against prod.
"""
...
TENANT_ID = 15
TENANT_NAME = "Dude Wipes"
TENANT_SLUG = "dude-wipes"
TEMPLATE_NAME = "Dude Wipes · Event Recap"
CODE_PREFIX = "DW-"
PROGRAM_NAME = "Event"
...
SPEC: list[tuple[str, list[tuple[str, str, bool, list[str]]]]] = [
    ...
    (
        ...
        [
            ...
            ("Consumer interaction count", "number", True, []),
    ...
    (
        ...
        [
            ...
            (
                "Event notes / Additional feedback",
                "longtext",
                False,
                [],
            ),
...
class Command(BaseCommand):
    help = (
        "Set up Dude Wipes: tenant (if missing), Event recap template, "
        "photo buckets, and standing check-in link (dry-run by default; --apply to write)."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default="",
            help=(
                "brand prefix for a NEWLY minted code, e.g. 'DW' -> "
                f"DW-XXXXXX. Blank keeps {CODE_PREFIX!r}."
            ),
        )
        parser.add_argument(
            "--apply",
    ...
    def _resolve_or_create_tenant(self, needle: str, creator, apply: bool):
        ...
        by_id = Tenant.objects.filter(id=TENANT_ID).first()
        if by_id:
            return by_id
        ...
        matches = list(
            Tenant.objects.filter(
                ...
                | Q(name__icontains="dude wipes")
                | Q(slug__icontains="dude-wipes")
            )
            .distinct()
            .order_by("id")
        )
    ...
    def _checkin_code(self, tenant, apply: bool, prefix: str = "") -> None:
        ...
        if raw and not 1 <= len(cleaned) <= 4:
            raise CommandError("--prefix should be 1-4 letters/digits, e.g. DW.")
