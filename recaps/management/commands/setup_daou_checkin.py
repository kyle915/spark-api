"""Make Treasury Wine Estates' ONE standing check-in link serve Event Activation.

DAOU-branded Event Activation walk-up: Liquid Death activation photo buckets
on a durable ``DAOU-`` URL. Recap form is seeded by ``seed_daou_recap_template``
— this command creates NEITHER the template nor a new code when one already
exists.

DRY-RUN by default. Run via ``/internal/cron/setup-daou-checkin`` (or the
"Setup DAOU check-in" GitHub Action) so it executes against prod.
"""
...
# Liquid Death / MAB Event Activation dropzones.
...
class Command(BaseCommand):
    help = (
        "Make Treasury Wine Estates / DAOU standing check-in serve Event "
        "Activation (photo buckets + selectable type; dry-run default)."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default="",
            help=(
                "brand prefix for a NEWLY minted code, e.g. 'DAOU' -> "
                f"DAOU-XXXXXX. Blank keeps {CODE_PREFIX!r}."
            ),
        )
        parser.add_argument(
            "--code",
    ...
    def _resolve_tenant(self, needle: str):
        ...
        matches = list(
            Tenant.objects.filter(
                ...
                | Q(name__icontains="treasury wine")
            )
            .distinct()
            .order_by("id")
        )
        if len(matches) > 1:
            prefer = [
                ...
                or (t.name or "").lower() == "treasury wine estates"
            ]
            if len(prefer) == 1:
                return prefer[0]
        if len(matches) == 1:
        ...
        if not matches:
            raise CommandError(
                f"No tenant matches {needle!r}. Onboard Treasury Wine Estates "
                "first (onboard_daou_products --create-tenant)."
            )
        raise CommandError(
    ...
    def _mint_code(self, prefix: str) -> str:
        ...
        if raw and not 1 <= len(cleaned) <= 4:
            raise CommandError("--prefix should be 1-4 letters/digits, e.g. DAOU.")
    ...
    def _ensure_photo_buckets(
        ...
        for _key, spec in bucket_plan.items():
    ...
    def handle(self, *args, **opts):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "APPLIED — DAOU Event Activation walk-up ready "
                "(code unchanged if it already existed)."
            )
        )
