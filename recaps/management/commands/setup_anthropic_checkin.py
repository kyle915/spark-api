"""Set up Anthropic: tenant + recap template + standing check-in.

Seeds the **Anthropic · BA Event Recap** form field-for-field off the
client's "BA EVENT RECAP" PDF (Leah Love, #1446, 08/19/2026). Date and
Event Location are NOT template fields — clock-in on the standing link
captures those on the event. Expense Receipts is intentionally omitted.

DRY-RUN by default. Run via ``/internal/cron/setup-anthropic-checkin`` (or
the "Setup Anthropic check-in" GitHub Action) so it executes against prod.
"""
...
TENANT_NAME = "Anthropic"
TENANT_SLUG = "anthropic"
TEMPLATE_NAME = "Anthropic · BA Event Recap"
CODE_PREFIX = "AN-"
PROGRAM_NAME = "Event"
...
# Pictures is a photo bucket, not a template image field — see PHOTO_BUCKETS.
...
class Command(BaseCommand):
    help = (
        "Set up Anthropic: tenant (if missing), BA Event Recap template, "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default="",
            help=(
                "brand prefix for a NEWLY minted code, e.g. 'AN' -> "
                f"AN-XXXXXX. Blank keeps {CODE_PREFIX!r}."
            ),
        )
        parser.add_argument(
            "--apply",
    ...
    def _checkin_code(self, tenant, apply: bool, prefix: str = "") -> None:
        ...
        if raw and not 1 <= len(cleaned) <= 4:
            raise CommandError("--prefix should be 1-4 letters/digits, e.g. AN.")
