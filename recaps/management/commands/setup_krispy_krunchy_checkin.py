"""Set up Krispy Krunchy Chicken: tenant + recap template + standing check-in.

The KKC twin of ``setup_feel_free_checkin`` — same two things in one
idempotent command, plus tenant creation because this brand may not exist
yet (Feel Free / Liquid Death already did):

1. The **Field Sampling Recap** custom template, field-for-field off the
   client's own "Field Sampling Recap // Krispy Krunchy Chicken" PDF
   (Anaelsa Ortiz, 02/16/2025). Date, City, Sampling Location #1, and the
   location-#1 "What time did you sample?" are NOT template fields: clock-in
   on the standing link already captures where/when the BA started, and the
   recap PDF already renders date/location from the event.
...
Receipts) rather than template image fields. Two image fields on one
template would share a single photo grid on the walk-up page; buckets are
how Liquid Death keeps those dropzones apart.

DRY-RUN by default. Run via ``/internal/cron/setup-krispy-krunchy-checkin``
(or the "Setup Krispy Krunchy Chicken check-in" GitHub Action) so it
...
TENANT_NAME = "Krispy Krunchy Chicken"
...
# time). Location #2 / #3 stay — clock-in is stop #1; extra stops are recap
# fields. Grammar on the two "How many consumer…" labels is corrected to
# "consumers" the same way Feel Free's "tasing" became "tasting".
#
# Location #2 / #3 counts are worded without "consumers did you sample" so
# they cannot steal the consumers-sampled KPI (first matching count wins).
SPEC: list[tuple[str, list[tuple[str, str, bool, list[str]]]]] = [
    ...
    (
        "Additional Stops",
        [
            ("Sampling Location #2", "text", False, []),
            ("What time did you sample at location #2?", "text", False, []),
            ("How many did you sample at location #2?", "number", False, []),
            ("Sampling Location #3", "text", False, []),
            ("What time did you sample at location #3?", "text", False, []),
            ("How many did you sample at location #3?", "number", False, []),
        ],
    ),
    (
        "Consumer Feedback",
...
class Command(BaseCommand):
    help = (
        "Set up Krispy Krunchy Chicken: tenant (if missing), field sampling "
        "recap template, and standing check-in link (dry-run by default; "
        "--apply to write)."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default="",
            help=(
                "brand prefix for a NEWLY minted code, e.g. 'KKC' -> "
                f"KKC-XXXXXX. Blank keeps {CODE_PREFIX!r}."
            ),
        )
        parser.add_argument(
            "--apply",
    ...
    def _checkin_code(self, tenant, apply: bool, prefix: str = "") -> None:
        ...
        if raw and not 1 <= len(cleaned) <= 4:
            raise CommandError("--prefix should be 1-4 letters, e.g. KKC.")
