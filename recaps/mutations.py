...
from recaps.mutation_parts.file_categories import (  # noqa: F401
    ...
    _resolve_role_file_recap_category,
)
...
from recaps.mutation_parts.notify import (  # noqa: F401
    ...
    _send_recap_data_quality_alert,
    _thread_recap_approved_notify,
)
from recaps.mutation_parts.exports import RecapExportMixin
...
class RecapMutationService(RecapExportMixin, SparkGraphQLMixin):
        ...
        for idx, field_input in enumerate(field_inputs):
            ...
            if custom_field_id is None:
                custom_field = models.CustomField.objects.create(
                    ...
                    required=bool(field_input.required),
                    options=list(field_input.options or []),
                    placeholder=(field_input.placeholder or "")
                    if getattr(field_input, "placeholder", None) is not None
                    else "",
                    order=order_val,
                )
            ...
            if getattr(field_input, "placeholder", None) is not None:
                custom_field.placeholder = field_input.placeholder or ""
    ...
    async def create_custom_recap(self) -> models.CustomRecap:
        ...
        # Approval is NEVER set on create/update submit. All tenants stay
        # Needs review until approveCustomRecap / approveRecap (which also
        # fire client-approved notify). Ignore any `approved` input here.
        @sync_to_async
    ...
    async def update_custom_recap(self) -> models.CustomRecap:
        ...
        # Approval is NEVER set on create/update submit — use
        # approveCustomRecap. Ignore any `approved` input on this path.
        @sync_to_async
        ...
        def create_custom_field_transaction():
            with transaction.atomic():
                custom_field = models.CustomField.objects.create(
                    ...
                    placeholder=(self.input.placeholder or "")
    ...
    async def update_custom_field(self) -> models.CustomField:
        ...
        def update_custom_field_transaction():
            with transaction.atomic():
                ...
                if self.input.placeholder is not None:
                    custom_field.placeholder = self.input.placeholder
    ...
    async def bulk_approve_recaps(self) -> int:
        ...
        if not isinstance(self.input, inputs.BulkApproveRecapsInput):
            raise GraphQLError("Invalid input type.")
        approved = bool(self.input.approved)
        count = 0
        for rid in self.input.recap_ids or []:
            svc = RecapMutationService.with_input(
                inputs.ApproveRecapInput(id=rid, approved=approved)
            )
            svc.user = self.user
            await svc.approve_recap()
            count += 1
        for rid in self.input.custom_recap_ids or []:
            svc = RecapMutationService.with_input(
                inputs.ApproveCustomRecapInput(id=rid, approved=approved)
            )
            svc.user = self.user
            await svc.approve_custom_recap()
            count += 1
        return count

    async def mark_recaps_shared(self) -> int:
        """Stamp shared_at so the pipeline can show Shared."""
        if not isinstance(self.input, inputs.MarkRecapsSharedInput):
            raise GraphQLError("Invalid input type.")
        ...
        for rid in custom_ids:
            try:
                pk = resolve_id_to_int(rid)
                recap = await sync_to_async(models.CustomRecap.objects.get)(id=pk)
            except Exception:
                raise GraphQLError("Custom recap not found.")
            await self._assert_caller_authorized_for_recap_tenant(
                recap.tenant_id,
        ...
        def _mark():
            ...
            for rid in recap_ids:
                pk = resolve_id_to_int(rid)
                recap = models.Recap.objects.get(id=pk)
                if recap.shared_at is None:
                    recap.shared_at = now
                    recap.updated_by = self.user
                    recap.save(update_fields=["shared_at", "updated_by", "updated_at"])
                n += 1
            for rid in custom_ids:
                pk = resolve_id_to_int(rid)
                recap = models.CustomRecap.objects.get(id=pk)
                if recap.shared_at is None:
                    recap.shared_at = now
                    recap.updated_by = self.user
                    recap.save(update_fields=["shared_at", "updated_by", "updated_at"])
    ...
    async def share_recap_by_email(self) -> tuple[str, int]:
        """Email the public /r/:token recap link to hand-picked recipients.

        Backs the on-platform Share modal (the replacement for the native
        OS share sheet). Returns (share_url, sent_count). Authorization
        mirrors mark_recaps_shared — admins any tenant, clients only their
        own, ambassadors blocked — plus the read-side client gate: a
        client-only caller can't share an unapproved recap (they can't
        even see one), so drafts never leak via a guessed id.
        """
        if not isinstance(self.input, inputs.ShareRecapByEmailInput):
            raise GraphQLError("Invalid input type.")

        if bool(self.input.recap_id) == bool(self.input.custom_recap_id):
            raise GraphQLError("Provide exactly one of recapId / customRecapId.")
        ...
        kind = "custom" if self.input.custom_recap_id else "legacy"
        if kind == "custom":
            try:
                pk = resolve_id_to_int(self.input.custom_recap_id)
                recap = await sync_to_async(
                    models.CustomRecap.objects.select_related(
                ...
                raise GraphQLError("Custom recap not found.")
            tenant_id = recap.tenant_id or (
                recap.event.tenant_id if recap.event_id else None
            )
            record_label = "Custom recap"
        else:
            try:
                pk = resolve_id_to_int(self.input.recap_id)
                recap = await sync_to_async(
                    models.Recap.objects.select_related(
                        "event", "event__tenant"
                    ).get
        ...
        # Client-only callers never share unapproved recaps — same posture
        # as the read resolvers (drafts return None for them). Admins keep
        # today's behavior: every recap is shareable.
        ...
        token = make_recap_token(kind, int(recap.id))
        share_url = f"{public_page_base()}/r/{token}"
        ...
        for recipient in clean:
            ...
            try:
                ...
                logger.exception(
                    "share_recap_by_email: send failed to %s recap=%s",
                    recipient,
        ...
        # Emailing the link IS sharing — stamp shared_at (same fields as
        # mark_recaps_shared) so the pipeline shows Shared.
        @sync_to_async
        def _stamp():
            type(recap).objects.filter(pk=recap.pk, shared_at__isnull=True).update(
                shared_at=django_timezone.now(), updated_by=self.user
            )
    ...
    async def submit_recap_client_signoff(self):
        ...
        if self.input.recap_id:
            try:
                pk = resolve_id_to_int(self.input.recap_id)
                recap = await sync_to_async(
                    models.Recap.objects.select_related(
                        "event", "event__rmm_asigned"
                    ).get
                )(id=pk)
            except Exception:
                raise GraphQLError("Recap not found.")
...
class RecapMutations:
    ...
    async def import_connecteam_recap_pdf(
        ...
        """Draft a CustomRecap from a Connecteam PDF.

        Preferred path: client PUTs the PDF to GCS via getUploadUrl, then
        sends pdfGcsPath. We enqueue the parse so a 100MB file never
        travels through GraphQL. pdfBase64 remains for tests / old clients.
        """
        import base64
        import uuid as _uuid

        from recaps.mutation_parts.connecteam import (
            enqueue_connecteam_import,
        ...
        gcs_path = (getattr(input, "pdf_gcs_path", None) or "").strip()
        if gcs_path:
            job_id = str(_uuid.uuid4())
            await sync_to_async(write_connecteam_job)(
                job_id, {"status": "pending", "message": "Importing…"}
            )
            await sync_to_async(enqueue_connecteam_import)(
                {
                    "job_id": job_id,
                    "blob_name": gcs_path,
                    "event_id": event.id,
        ...
        if not input.pdf_base64:
            return build_mutation_response(
                types.ImportConnecteamRecapPdfResponse,
                success=False,
                message="Upload the PDF to storage first (pdfGcsPath), or send pdfBase64.",
                input_obj=input,
            )
        ...
        return await execute_connecteam_import_from_bytes(
            user=user,
            event=event,
            template=template,
            pdf_bytes=pdf_bytes,
    ...
    async def bulk_approve_recaps(
        self,
        info: strawberry.Info,
        input: inputs.BulkApproveRecapsInput,
    ) -> types.BulkRecapsResponse:
        try:
            service = RecapMutationService.with_input(input)
            await service.set_user(info)
            updated = await service.bulk_approve_recaps()
            return build_mutation_response(
                types.BulkRecapsResponse,
                success=True,
                message=f"Updated {updated} recap(s).",
                input_obj=input,
    ...
    @relay.mutation(permission_classes=[StrictIsAuthenticated])
    async def mark_recaps_shared(
        self,
        info: strawberry.Info,
        input: inputs.MarkRecapsSharedInput,
    ) -> types.BulkRecapsResponse:
        try:
            service = RecapMutationService.with_input(input)
            await service.set_user(info)
            updated = await service.mark_recaps_shared()
            return build_mutation_response(
                types.BulkRecapsResponse,
                success=True,
                message=f"Marked {updated} recap(s) shared.",
                input_obj=input,
                updated_count=updated,
            )
        except GraphQLError as e:
    ...
    @relay.mutation(permission_classes=[StrictIsAuthenticated])
    async def share_recap_by_email(
        self,
        info: strawberry.Info,
        input: inputs.ShareRecapByEmailInput,
    ) -> types.ShareRecapByEmailResponse:
        """Email the public /r/:token recap link from the Share modal."""
        try:
            service = RecapMutationService.with_input(input)
            await service.set_user(info)
            share_url, sent = await service.share_recap_by_email()
            return build_mutation_response(
                types.ShareRecapByEmailResponse,
                success=True,
                message=f"Recap link emailed to {sent} recipient(s).",
                input_obj=input,
                share_url=share_url,
                sent_count=sent,
            )
    ...
    @relay.mutation(permission_classes=[StrictIsAuthenticated])
    async def submit_recap_client_signoff(
        self,
        info: strawberry.Info,
        input: inputs.RecapClientSignoffInput,
    ) -> types.RecapClientSignoffResponse:
        try:
            service = RecapMutationService.with_input(input)
            await service.set_user(info)
            kind, recap = await service.submit_recap_client_signoff()
            kwargs = {"recap": recap} if kind == "legacy" else {"custom_recap": recap}
            return build_mutation_response(
                types.RecapClientSignoffResponse,
                success=True,
                message="Sign-off saved.",
                input_obj=input,
                **kwargs,
            )
        except GraphQLError as e:
