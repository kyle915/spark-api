/**
 * Liquid Death Retail Market Schedules → Spark Event Confirmation / Cancel
 *
 * Install on:
 *   https://docs.google.com/spreadsheets/d/1P1k5bpGuce_qAeh1qg9kNI4QjzRYLZhHTDm2vnQQKi0
 *   (gid 0 retail schedule tab)
 *
 ...
 *
 * Live retail tab (gid 0), read 2026-09-18 — headers A–AA end at Kyle Check.
 * After ensureConfirmationColumns (append-only), expect:
 *   AB Event Confirmation Sent?
 *   AC Send Confirmation
 *   AD Cancel Confirmation
 *   AE Force Resend
 *   AF Confirmation Status
...
var LD_SHEET_ID = '1P1k5bpGuce_qAeh1qg9kNI4QjzRYLZhHTDm2vnQQKi0';
var LD_SHEET_GID = 0;
var DEFAULT_API_BASE =
  'https://spark-api-new-490085168610.us-central1.run.app';
var ENDPOINT_PATH = '/internal/liquid-death-sheet-event-confirmation';

var HEADER_SENT_STATUS = 'Event Confirmation Sent?';
...
function onConfirmationEdit(e) {
  ...
  if (sheet.getParent().getId() !== LD_SHEET_ID) return;
...
function _retailScheduleSheet_() {
  var ss = SpreadsheetApp.openById(LD_SHEET_ID);
  var sheets = ss.getSheets();
  for (var i = 0; i < sheets.length; i++) {
    if (sheets[i].getSheetId() === LD_SHEET_GID) return sheets[i];
  }
  return sheets[0];
}
