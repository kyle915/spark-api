...
class SeedNeutonicRecapTemplateView(View):
    ...
    Builds/reconciles Neutonic's **Event Activation** recap template (MAB/LD
    Event Activation questions with Neutonic brand copy; Products Sampled from
    the live Neutonic Product catalog). Does NOT touch Retail / Event /
    On-Premise forms. Fires ``seed_neutonic_recap_template``. DRY-RUN unless
    apply=true.

    Params: tenant (default "neutonic"), template_name, event_type, apply.
    """
    ...
    def _run(self, request: HttpRequest) -> HttpResponse:
        ...
        apply_raw = (_param("apply") or "").lower()
        kwargs: dict = {
            "tenant": _param("tenant") or "neutonic",
            "apply": apply_raw in ("1", "true", "yes", "on"),
        }
        if _param("template_name"):
            kwargs["template_name"] = str(_param("template_name"))
        ...
        out = io.StringIO()
        try:
            call_command("seed_neutonic_recap_template", stdout=out, **kwargs)
        except Exception as exc:  # noqa: BLE001 — surface to caller
            logger.exception("seed_neutonic_recap_template cron failed")
            return JsonResponse(
                {
                    "ok": False,
                    "error": "command-failed",
...
class SetupNeutonicCheckinView(View):
    ...
    Params: tenant (default "neutonic"), prefix, code, apply (default DRY RUN).
    """
    ...
    def _run(self, request: HttpRequest) -> HttpResponse:
        ...
        out = io.StringIO()
        try:
            call_command("setup_neutonic_checkin", stdout=out, **kwargs)
        except Exception as exc:  # noqa: BLE001 — surface to caller
            logger.exception("setup-neutonic-checkin cron failed")
            return JsonResponse(
                {"ok": False, "error": "command-failed", "detail": str(exc),
                 "log": out.getvalue()},
                status=500,
...
@method_decorator(csrf_exempt, name="dispatch")
class SetupLuckinCheckinView(View):
    """GET/POST `/internal/cron/setup-luckin-checkin`.

    Creates the Luckin Coffee tenant if missing, seeds the Sampling Recap
    template off the client's Connecteam PDF fields, mints the standing
    ``LC-`` check-in code, pins Event, and labels the photo bucket.

    Params: tenant (default "luckin"), template_name, event_type, prefix,
    apply (default DRY RUN).
    """
    ...
    def _run(self, request: HttpRequest) -> HttpResponse:
        ...
        out = io.StringIO()
        try:
            call_command("setup_luckin_checkin", stdout=out, **kwargs)
        except Exception as exc:  # noqa: BLE001 — surface to caller
            logger.exception("setup-luckin-checkin cron failed")
            return JsonResponse(
                {"ok": False, "error": "command-failed", "detail": str(exc),
                 "log": out.getvalue()},
                status=500,
...
@method_decorator(csrf_exempt, name="dispatch")
class SetupBrandRevolutionCheckinView(View):
    """GET/POST `/internal/cron/setup-brand-revolution-checkin`.

    Creates the Brand Revolution tenant if missing (createTenant-style seeds),
    seeds the Event Activation recap (interact + survey counts), labels the
    Recap pictures photo bucket, mints the standing ``BR-`` check-in code, and
    pins Event Activation as the only selectable program.

    Idempotent: get_or_create for the template, and an existing checkin_code
    is left alone (rotating it breaks every copy already shared). Recaps stay
    human-reviewed (walk-up leaves approved=False).

    Params: tenant (default "brand-revolution"), template_name, event_type,
    ...
    def _run(self, request: HttpRequest) -> HttpResponse:
        ...
        out = io.StringIO()
        try:
            call_command("setup_brand_revolution_checkin", stdout=out, **kwargs)
        except Exception as exc:  # noqa: BLE001 — surface to caller
            logger.exception("setup-brand-revolution-checkin cron failed")
            return JsonResponse(
                {"ok": False, "error": "command-failed", "detail": str(exc),
                 "log": out.getvalue()},
                status=500,
...
class SetupSipliCheckinView(View):
    ...
    The Sipli twin of setup-g7-entertainment-checkin: creates the tenant if
    missing (createTenant-style seeds), seeds Event Sampling Recap (PDF minus
    Date / Sampling Location) plus a Retail Sampling twin (Store Number +
    inventory), mints the standing ``SIP-`` check-in code, and makes Retail
    Sampling vs Event Activation selectable on that one link (Liquid Death
    picker). Recaps stay human-reviewed. Location mode is Event-style
    address/GPS.

    Idempotent: get_or_create for the templates, and an existing checkin_code
    is left alone (rotating it breaks every copy already shared).

    Params: tenant (default "sipli"), prefix, apply (default DRY RUN).
    """
    ...
    def _run(self, request: HttpRequest) -> HttpResponse:
        ...
        out = io.StringIO()
        try:
            call_command("setup_sipli_checkin", stdout=out, **kwargs)
        except Exception as exc:  # noqa: BLE001 — surface to caller
            logger.exception("setup-sipli-checkin cron failed")
            return JsonResponse(
                {"ok": False, "error": "command-failed", "detail": str(exc),
                 "log": out.getvalue()},
                status=500,
...
@method_decorator(csrf_exempt, name="dispatch")
class SetupPivotEnergyCheckinView(View):
    """GET/POST `/internal/cron/setup-pivot-energy-checkin`.

    Creates the Pivot Energy Drink tenant if missing (createTenant-style seeds),
    seeds the Event Activation recap template
    ("Pivot Energy Drink · Event Activation Recap") mirroring Breakaway WCS
    structure with Pivot field renames, mints the standing ``PE-`` check-in
    code, pins Event Activation, and labels Consumer Sampling Pictures +
    Expense Receipts.
    ...
    Params: tenant (default "pivot-energy-drink"), template_name, event_type,
    prefix, apply (default DRY RUN).
    """
    ...
    def _run(self, request: HttpRequest) -> HttpResponse:
        ...
        out = io.StringIO()
        try:
            call_command("setup_pivot_energy_checkin", stdout=out, **kwargs)
        except Exception as exc:  # noqa: BLE001 — surface to caller
            logger.exception("setup-pivot-energy-checkin cron failed")
            return JsonResponse(
                {"ok": False, "error": "command-failed", "detail": str(exc),
                 "log": out.getvalue()},
                status=500,
...
class SetupDudeWipesCheckinView(View):
    ...
    Creates the Dude Wipes tenant if missing (createTenant-style seeds),
    seeds the Event recap template ("Dude Wipes · Event Recap") mirroring
    White Claw Surge structure with all Dude Wipes branding, mints the
    standing ``DW-`` check-in code, pins Event, and labels photo buckets.

    Idempotent: get_or_create for the template, and an existing checkin_code
    is left alone (rotating it breaks every copy already shared). Recaps stay
    human-reviewed (walk-up leaves approved=False).

    Params: tenant (default "dude-wipes"), template_name, event_type,
    prefix, apply (default DRY RUN).
    """
    ...
    def _run(self, request: HttpRequest) -> HttpResponse:
        ...
        out = io.StringIO()
        try:
            call_command("setup_dude_wipes_checkin", stdout=out, **kwargs)
        except Exception as exc:  # noqa: BLE001 — surface to caller
            logger.exception("setup-dude-wipes-checkin cron failed")
            return JsonResponse(
                {"ok": False, "error": "command-failed", "detail": str(exc),
                 "log": out.getvalue()},
                status=500,
...
@method_decorator(csrf_exempt, name="dispatch")
class RenameTorchOnshelfBucketView(View):
    """GET/POST `/internal/cron/rename-torch-onshelf-bucket`.

    Torch THC: rename FileRecapCategory + checkin_photo_buckets from
    "On-Shelf Product" to "Before & After Shelf / Stock" in place so
    historical recaps stay mapped. Does not remint TH-2HRV3D.
    ...
    def _run(self, request: HttpRequest) -> HttpResponse:
        ...
        out = io.StringIO()
        try:
            call_command("rename_torch_onshelf_bucket", stdout=out, **kwargs)
        except Exception as exc:  # noqa: BLE001 — surface to caller
            logger.exception("rename-torch-onshelf-bucket cron failed")
            return JsonResponse(
                {
                    "ok": False,
                    "error": "command-failed",
...
def _registered_views() -> dict[str, Any]:
    ...
    return {
        ...
        "setup-brand-revolution-checkin": SetupBrandRevolutionCheckinView,
        ...
        "setup-pivot-energy-checkin": SetupPivotEnergyCheckinView,
